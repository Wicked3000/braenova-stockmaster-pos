...
def test_api():
    # Register via web
    r = requests.post('http://127.0.0.1:5000/register', data={'shop_name':'Test', 'username':'testuser99', 'password':'password'})
    print('Web register:', r.status_code)
    
    # Login via API
    r = requests.post('http://127.0.0.1:5000/api/v1/auth/login', json={'username':'testuser99','password':'password'})
    data = r.json()
